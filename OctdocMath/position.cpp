...
namespace octdoc
{
	namespace mth
	{
		...
		{
			MoveInLookDirection(mth::float3(0.0f, 0.0f, d));
		}
		void Position::MoveInLookDirection(float3 delta)
		{
			position += float3x3::Rotation(rotation) * delta;
		}
		...
		float4x4 Position::GetPositionMatrixInv()
		{
			return float4x4::Translation(-position);
		}
		float4x4 Position::GetRotationMatrixInv()
		{
			return float4x4::Rotation(-rotation);
		}
		...
		{
			return float3x3::Rotation(rotation) * float3(0.0f, 0.0f, 1.0f);
		}
		void Position::Reset()
		{
			position = 0.0f;
